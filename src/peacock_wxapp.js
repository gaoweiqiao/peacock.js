...
(function(module){
    ...
    SignalCenter.prototype.on = function(signalName,listener,context){
        console.log(context);
        ...
        if(!signalTable.hasOwnProperty(signalName)){
            ...
            if(-1 == listenerList.indexOf(listener)){
                listenerList.push(listener);
            }
        }
    };
    SignalCenter.prototype.un = function(signalName,listener){
        if(signalTable.hasOwnProperty(signalName)){
            var listenerList = signalTable[signalName];
            var index = listenerList.indexOf(listener);
            if(-1 !== index){
                listenerList.splice(index,1);
                if(0 === listenerList.length){
                    delete signalTable[signalName];
                }
            }
        }
    };
    SignalCenter.prototype.send = function(signalName){
        if(signalTable.hasOwnProperty(signalName)){
            var listenerList = signalTable[signalName];
    ...
    function Database(){
        ...
        Operation.prototype.execute = function(){
            ...
            if(isFunction(this.comparator)){
                data = sort(data,this.comparator);
            }
        ...
        this.constructor.prototype.insertBatch = function(dataList){
            if(Array.isArray(dataList)){
                return new InsertOperation(dataList,true);
            }else{
    ...
    module.config({
        ...
        '$bus': function(){return module.$bus},

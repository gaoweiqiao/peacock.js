...
pck.use(['$date'],function($date){
    ...
    assert({
        test:function(){
            //
            var date = new Date(2016,10,22,21,24,1,456);
            return (
                date.getTime() === $date.parse('yyyy-MM-dd HH:mm:ss z','2016-11-22 21:24:1 456').getTime()
            );
        },
        success:'$date.parseDate test pass'
    });
});
/**
 * 测试 $string
...
pck.use(['$db'],function($db){
    ...
    var result = $db.select().from('test').execute();
    $db.update(function(item){
        item.age++;
    }).from('test').where(condition).execute();
    $db.delete().from('test').where(condition).execute();
});
